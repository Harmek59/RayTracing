...
Camera::Camera(uint32_t screenWidth, uint32_t screenHeight, float fov, float nearPlane, float farPlane) {
    ...
    setUpProjection(glm::radians(fov), static_cast<float>(screenWidth) / static_cast<float>(screenHeight), nearPlane,
                    farPlane);
    update();
}
...
void Camera::setScreenSize(uint32_t screenWidth, uint32_t screenHeight) {
    setUpProjection(this->fov, static_cast<float>(screenWidth) / static_cast<float>(screenHeight), this->nearPlane,
                    this->farPlane);
}
...
void Camera::rotate(float yaw, float pitch, float roll, bool constrainPitch) {
    this->yaw += yaw;
    this->pitch += pitch;
    this->roll += roll;
...
void Camera::setFirstMouseMove(bool val) {
    firstMouseMove = val;
}