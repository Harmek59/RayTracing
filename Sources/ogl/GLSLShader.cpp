...
GLSLShader::GLSLShader(const std::string &vertexShaderPath, const std::string &fragmentShaderPath,
...
GLSLShader::GLSLShader(const std::string &vertexShaderPath, const std::string &fragmentShaderPath,
    ...
    GLuint shader = createAndCompileShader(readShaderFromFile(vertexShaderPath), GL_VERTEX_SHADER,
                                           "Vertex Shader: " + vertexShaderPath);
    attachShaderToProgram(shader, GL_VERTEX_SHADER);

    shader = createAndCompileShader(readShaderFromFile(fragmentShaderPath), GL_FRAGMENT_SHADER,
                                    "Fragment Shader: " + fragmentShaderPath);
    attachShaderToProgram(shader, GL_FRAGMENT_SHADER);

    if (tessControlShaderPath != "") {
        shader = createAndCompileShader(readShaderFromFile(tessControlShaderPath), GL_TESS_CONTROL_SHADER,
                                        "Tesselation control Shader: " + tessControlShaderPath);
        attachShaderToProgram(shader, GL_TESS_CONTROL_SHADER);
    }
    if (tessEvaluationShaderPath != "") {
        shader = createAndCompileShader(readShaderFromFile(tessEvaluationShaderPath), GL_TESS_EVALUATION_SHADER,
                                        "Tesselation evaluation Shader: " + tessEvaluationShaderPath);
        attachShaderToProgram(shader, GL_TESS_EVALUATION_SHADER);
    }
    if (geometryShaderPath != "") {
        shader = createAndCompileShader(readShaderFromFile(geometryShaderPath), GL_GEOMETRY_SHADER,
                                        "Geometry Shader: " + geometryShaderPath);
...
GLSLShader::GLSLShader(const std::string &computeShaderPath) {
    ...
    GLuint shader = createAndCompileShader(readShaderFromFile(computeShaderPath), GL_COMPUTE_SHADER,
...
GLuint
GLSLShader::createAndCompileShader(const std::string &shaderCode, GLenum shaderType, const std::string &shaderName) {
