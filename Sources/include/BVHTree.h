...
public:
    ...
    void createFromModels(const std::vector<Model> &modelsArray) {
        ...
        for (int i = 0; i < modelsArray.size(); ++i) {
            ...
            auto[bbMin, bbMax] = modelsArray[i].triangleMesh.getBoundingBox();
            ...
            bbMin = min;
            bbMax = max;
            modelsBB.push_back(AABB{bbMin, i, bbMax, 1});
        }
...
//                  << " | max Axis: " << longestAxis << "\n";
        ...
        int splitPoint = modelsBBs.size() / 2;
        ...
        if (sahSplit) {
            ...
            for (int splitAfter = 0; splitAfter < modelsBBs.size() - 1; splitAfter++) {
                ...
                for (int i = 1; i <= splitAfter; i++) {
                    ...
                    if (modelsArray[i].grid.getNumberOfCells()) {
                        std::cout << "DUUUUPA";
                    }
                }
                float leftNumberOfCells =
                        (left.end - left.begin).x * (left.end - left.begin).y * (left.end - left.begin).z;
                ...
                float rightNumberOfCells =
                        (right.end - right.begin).x * (right.end - right.begin).y * (right.end - right.begin).z;
